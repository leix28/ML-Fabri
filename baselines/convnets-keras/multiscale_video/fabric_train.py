...
with tf.device('/gpu:0'):
    ...
    model = AlexNet(nb_classes=nb_classes, sz=sz, channel=img_channels)
...
if data_augmentation:
    ...
    def predict(model, val=True):
        y_preds = []
        Z = []
        for (x, y, z) in datagenerator.test_generator(dataset_fn, imgs_fn, val=val, sz=img_rows, rescale=rescale):
    ...
    G = data_gen('../../../data_preprocessing/material_dataset.txt', batch_size=batch_size, datagen=datagen, sz=sz, rescale=rescale)
