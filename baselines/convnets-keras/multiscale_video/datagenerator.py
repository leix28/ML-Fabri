...
def test_generator(datasplit, feature, batch_size=32, val=True, nb_classes=14, sz=227, rescale=[1.0]):
        ...
        for p in points:
            ...
            for r in rescale:
                ...
                if r == 4:
                    nx = max(x-hf*3, 0)
                    ny = max(y-hf*3, 0)
                    ims.append(cv2.resize(im[nx:nx+s, ny:ny+s], (sz, sz)))
                else:
                    ims.append(cv2.resize(im[x:x+s, y:y+s], (sz, sz)))
            ret.append(np.concatenate(ims, axis=2))
        return ret
...
def data_gen(datasplit, d=300, sz=227, batch_size=32, datagen=None, nb_classes=14, rescale=[1.0]):
    gc.collect()

    def f(img):
        #return cv2.resize(img[360-d:360+d, 480-d:480+d], (224, 224))
        maxs = int(np.max(rescale) * sz)


        ims = []
        for r in rescale:
            s = int(r*sz)
            x = random.randint(360-d, 360+d-s)
            y = random.randint(480-d, 480+d-s)
            if s == 0:
                im = cv2.resize(img, (sz, sz))
            else:
                im = cv2.resize(img[x:x+s, y:y+s], (sz, sz))
            ims.append(im)
