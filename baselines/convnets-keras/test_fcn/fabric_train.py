...
with tf.device('/gpu:0'):
    ...
    #model, _ = AlexNet(weights_fn, nb_classes=nb_classes, sz=sz)
    model, _ = AlexNet(weights_fn, nb_classes=nb_classes, sz=sz)
    model_test, outs = AlexNet(nb_classes=nb_classes, sz=test_sz, test=True)
...
def sum_loss(y_true, y_pred):
    y_true = K.reshape(y_true, [batch_size, nb_classes])
    y_pred = K.reshape(y_pred, [batch_size, nb_classes])
    s = K.mean(K.categorical_crossentropy(y_true, y_pred))
    return s

#opt = Adadelta(lr=0.01, rho=0.95, epsilon=1e-08, decay=0.0)
#opt = Adadelta(lr=0.1, rho=0.95, epsilon=1e-08, decay=0.0)
...
model_test.compile(loss=sum_loss,
            optimizer=opt,
            metrics=['accuracy'])
...
if data_augmentation:
    ...
    def predict(model, val=True):
        y_preds = []
        Z = []
        for (x, y, z) in datagenerator.test_generator(dataset_fn, imgs_fn, val=val, sz=test_sz):
            y_pred = model_test.predict(x, batch_size=batch_size)
