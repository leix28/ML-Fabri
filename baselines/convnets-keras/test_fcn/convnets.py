...
from keras.layers import Conv2D as Conv2D_
import numpy as np
...
def Conv2D(c, k, activation='relu', name='', strides=(1, 1)):
    global model_
    try:
        layer = model_.get_layer(name)
    except:
        layer = Conv2D_(c, k, strides=strides, activation=activation, name=name)
    return layer


def AlexNet(weights_path=None, heatmap=False, nb_classes=14, sz=227, test=False):
    global model_
    if not(test):
        model_ = None
    ...
    dense_1_conv = MaxPooling2D((3, 3), strides=(2, 2))(conv_5)
    print dense_1_conv.shape
    dense_1_conv = Conv2D(4096, (6, 6), activation='relu', name='dense_1_conv')(dense_1_conv)
    dense_1_conv = Dropout(0.5)(dense_1_conv)
    dense_2_conv = Conv2D(4096, (1, 1), activation='relu', name='dense_2_conv')(dense_1_conv)
    dense_2_conv = Dropout(0.5)(dense_2_conv)
    dense_3_conv = Conv2D(nb_classes, (1, 1), name='dense_3_conv')(dense_2_conv)
    prediction_new = Softmax4D(axis=-1, name='softmax')(dense_3_conv)

    if not(test):
        prediction_new = Flatten()(prediction_new)
        outs = 1
    else:
        outs = int(prediction_new.shape[1])


    model = Model(inputs=inputs, outputs=prediction_new)
    print prediction_new.shape

    print model.get_layer('dense_2_conv')
