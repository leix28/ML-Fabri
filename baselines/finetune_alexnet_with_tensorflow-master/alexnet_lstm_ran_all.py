...
class Dataset(object):
    def __init__(self, sets=[0, 1, 2]):
        ...
        def genlist(data):
            ...
            for i in range(n - SEQ_LEN + 1):
                if not data[i][0][:-6] in pressdic:
                    pressdic[data[i][0][:-6]] = npresses
                    npresses += 1
                if data[i][0][:-6] == data[i+SEQ_LEN-1][0][:-6]:
                    lst.append(i)
                    pressid.append(pressdic[data[i][0][:-6]])
                
            return lst, 

        self.train_examples, _ = genlist(self.train)
        self.val_examples, _ = genlist(self.val)
        self.test_examples, self.test_pressid = genlist(self.test)
        self.test_examples = self.test_examples * TEST_RAN
        self.test_pressid = self.test_pressid * TEST_RAN
    ...
    def get_test_max_id(self):
        return np.max(self.test_pressid)
    ...
    def get_test(self, idx):
        ...
        return imgs.reshape((BATCH_SIZE * SEQ_LEN, 227, 227, 3)), lb, self.test_pressid[data]
